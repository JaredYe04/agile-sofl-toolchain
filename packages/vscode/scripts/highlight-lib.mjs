...
const onigPkgRoot = dirname(require.resolve('vscode-oniguruma/package.json'))
...
async function getRegistry() {
  if (!registryPromise) {
    registryPromise = (async () => {
      const wasmPath = join(onigPkgRoot, 'release', 'onig.wasm')
      const wasm = readFileSync(wasmPath)
      await loadWASM(wasm.buffer)
      const grammarPath = join(vscodeRoot, 'syntaxes', 'agile-sofl.tmLanguage.json')
