...
import type { WorkspaceTreeActionId, WorkspaceTreeMenuItem } from '../../workspaceTree/types'

const props = defineProps<{
  open: boolean
  x: number
  y: number
  items: WorkspaceTreeMenuItem[]
}>()

const emit = defineEmits<{
  pick: [action: WorkspaceTreeActionId]
  close: []
}>()
...
<template>
  <Teleport to="body">
    <div
      v-if="open"
      class="fixed z-[200] min-w-[180px] select-none rounded-md border border-border-subtle bg-surface-raised py-1 shadow-lg"
