...
- 侧栏为**增强配对视图**（非完整目录树）：每组 informal / hybrid / GUI 三行，未配对文件按扩展名分组；可折叠、可拖拽调宽（200–420px）、右键菜单（打开/在资源管理器中显示/复制路径）
...
| P4 | 全链路测试、手测清单 | ✅ |
| P5 | GUI 可交互原型、Coverage gui-screen | ✅ |
| P6 | ASFL gui 文法附挂解析、精化 emitGuiBlock | ✅ |
...
- ~~可交互 GUI 原型（Preview/Edit、屏间导航、控件选中）~~ ✅
- ~~ASFL gui 文法（Chevrotain 原生规则、`check()` 诊断、精化 `emitGuiBlock`、LSP 补全、Hybrid GUI 表单）~~ ✅（见 [19-ASFL-GUI文法扩展.md](./19-ASFL-GUI文法扩展.md)）
