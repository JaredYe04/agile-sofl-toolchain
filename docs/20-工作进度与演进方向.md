...
| 非形式规格 `.aspec` | **已完成（Markdown 主格式）** | 用 Functions / Data Resources / Constraints 组织自然语言需求 |
| Informal → Hybrid 精化 | **已完成 + 可插拔 Generator** | 规则基线 + ChatECNU LLM 基线，研究方可注册自己的生成器 |
| Studio 可视化编辑 | **已完成** | Informal Document/Graphical 双视图；Hybrid Code/Visual 共用模型 |
| Studio 四栏工作区 | **已完成** | 项目树、Informal+Agent、Hybrid+GUI、规格结构树 |
| AI Specification Agent | **已完成（ChatECNU）** | 需求分析、澄清卡片、Patch 预览后写入，不直接覆盖文件 |
